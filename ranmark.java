...
public class ranmark implements ActionListener{
    Frame win = new Frame();
    ...
    public void actionPerformed(ActionEvent e){
        ...
        int b = ob.nextInt(100);
        int c = ob.nextInt(100);
        int d = ob.nextInt(100);
        int f = ob.nextInt(100);
        int g = ob.nextInt(100);

        int obta = b+c+d+f+g;
        int per = obta*100/500;
        if (e.getSource()==b1) {
            
        
        if (a!=500) {
            JOptionPane.showMessageDialog(win, "INVALID TOTAL MARKS");
        }
        else
            if (a==500) {
                t2.setText(""+b);
